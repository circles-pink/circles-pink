...
instance ToTsType Number where
  toTsType _ = TS.number

instance ToTsType Int where
  toTsType _ = TS.mkType_ $ TS.qualName "PursTsGen_Prim" "Int"

instance ToTsType String where
  toTsType _ = TS.string

instance ToTsType Boolean where
  toTsType _ = TS.boolean

instance ToTsType a => ToTsType (Array a) where
  toTsType _ = TS.array $ toTsType (undefined :: a)

instance (RowToList r rl, GenRecord rl) => ToTsType (Record r) where
  toTsType _ = TS.record $ genRecord (Proxy :: _ rl)

instance (ToTsType a, ToTsType b) => ToTsType (a -> b) where
  toTsType _ = TS.function_
    [ TS.keyVal "_" $ toTsType (undefined :: a) ]
    (toTsType (undefined :: b))

instance ToTsType a => ToTsType (Proxy a) where
  toTsType _ = toTsType (undefined :: a)

instance (RowToList r rl, GenVariant rl) => ToTsType (Variant r) where
...
instance (GenRecord rl, ToTsType t, IsSymbol s) => GenRecord (Cons s t rl) where
  genRecord _ =
...
instance (ToTsType t, IsSymbol s) => GenVariant (Cons s t Nil) where
  genVariant _ = TS.record'
    { type: TS.tlString (reflectSymbol (Proxy :: _ s))
    , value: toTsType (Proxy :: _ t)
    }

else instance (GenVariant rl, ToTsType t, IsSymbol s) => GenVariant (Cons s t rl) where
  genVariant _ =
...
instance ToTsType (Function a b) => ToTsType (Constructor (Function a b)) where
  toTsType (Constructor f) = TS.record' { create: toTsType f }

else instance ToTsType a => ToTsType (Constructor a) where
...
instance ToTsType a => ToTsPredFn (a -> Boolean) where
  toTsPredFn t _ = TS.function_
    [ TS.keyVal "obj" $ toTsType (undefined :: a) ]
    (TS.isPred (TS.name "obj") t)

else instance (ToTsType a, ToTsPredFn b) => ToTsPredFn (a -> b) where
  toTsPredFn t _ = TS.function_
    [ TS.keyVal "_" $ toTsType (undefined :: a) ]
